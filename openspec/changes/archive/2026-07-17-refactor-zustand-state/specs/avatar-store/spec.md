...
AIGC:
  ...
  Label: '1'
  ProduceID: 'fa1cf91d-597a-47f2-99d4-0e08de51bb78'
  PropagateID: 'fa1cf91d-597a-47f2-99d4-0e08de51bb78'
  ReservedCode1: '47401533-666b-46d5-b03e-88148bfb39a8'
  ReservedCode2: '47401533-666b-46d5-b03e-88148bfb39a8'
---
...
以 zustand 全局 store 承载像素头像生成器的全部应用状态（画板数据、工具、颜色、镜像、历史栈、风格等），让组件按需精确订阅，取代此前在 `usePixelArt` hook 中集中管理并通过 props 逐层下传的模式。
...
- TypeScript strict，store 类型显式声明为 `AvatarState`
- 不新增状态字段或新功能，行为与迁移前逐项等价
- 不改动 `utils/canvas.ts`、`utils/dicebear.ts`、`types.ts`、`constants.ts` 及任何 CSS 文件
